...
1. **`crates/server`** — a small [axum](https://github.com/tokio-rs/axum) **backend-for-frontend (BFF)**. It embeds `desktop-assistant`'s own `ws-interface` WebSocket server as the browser-facing front door (reusing `/ws`, `/login`, `/auth/config` + JWT, not reimplementing them), and connects to the local daemon over **UDS** via `client-common`'s `Connector` as the back door. The only substantial new logic is a `ForwardingHandler: AssistantApiHandler` that bridges the two.
